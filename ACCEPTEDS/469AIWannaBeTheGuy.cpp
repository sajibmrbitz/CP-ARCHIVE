...
cin>>n;
vector<int>levels(n);
for(int i=0;i<n;i++)    levels[i]=i+1;
int c,d;
cin>>c;
vector<int> canx(c);
for(int &x:canx)    cin>>x;
cin>>d;
vector<int> cany(d);
for(int &y:cany)    cin>>y;
bool pass=true;

for(int i=0;i<n;i++){
    bool ok=false;
    for(int x:canx)if(x==levels[i]) ok=true;
    for(int y:cany)if(y==levels[i]) ok=true;
    if(!ok){
        pass=false;
        break;
    }
}
cout<<(pass?"I become the guy.":"Oh, my keyboard!");
return 0;
}
...
the indices of levels Little Y can pass. Will Little X and Little Y pass 
...
The next line contains the levels Little Y can pass in the same format. It's assumed that levels 
