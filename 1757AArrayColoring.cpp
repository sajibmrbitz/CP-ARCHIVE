...
void solve(){
    ...
    if(prefixsum[n-1]%2)    cout<<"NO"<<endl;
    else                    cout<<"YES"<<endl;
}
...
// Archive: CF1857A_EqualParityColoring
// Verdict: Accepted
// Insight: Total sum parity determines feasibility
// Tags: parity, greedy, math, implementation




/*
You are given an array consisting of n integers. Your task is to determine whether 
it is possible to color all its elements in two colors in such a way that the sums of the elements 
of both colors have the same parity and each color has at least one element colored.

For example, if the array is [1,2,4,3,2,3,5,4], we can color it as follows: 
[1,2,4,3,2,3,5,4], where the sum of the blue elements is 6 and the sum of the red elements is 18.
